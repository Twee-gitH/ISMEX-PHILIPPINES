...
st.markdown("""
    ...
    /* STYLE FOR THE REFERRAL LINK BOX */
    .ref-box {
        background: #111; padding: 10px; border-radius: 8px; border: 1px dashed #00ff88; 
...
# --- USER DASHBOARD ---
    ...
    # REFERRAL LINK GENERATOR (ADDED)
    st.markdown('<div class="ref-box"><p style="color:#8c8f99; font-size:12px; margin:0;">🤝 YOUR REFERRAL LINK</p></div>', unsafe_allow_html=True)
    my_link = f"https://twee-gith.github.io/ISMEX-PHILIPPINES/?ref={st.session_state.user.replace(' ', '+')}"
    ...
    if not active: st.info("No running capitals.")
        ...
        for idx, a in reversed(list(enumerate(active))):
            ...
            if st.button(f"📥 PULL OUT ₱{total_roi:,.2f}", key=f"p_{idx}", disabled=not is_clickable):
                ...
                data.setdefault('history', []).append({
                    "type": "PULL_OUT", "amount": total_roi, "date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "status": "CONFIRMED"
                })
                active.pop(idx); update_user(st.session_state.user, data); st.rerun()

    # REFERRAL COMMISSIONS (UNTOUCHED)
    ...
    if not comms: st.info("No referral commissions yet.")
        ...
        for idx, c in enumerate(comms):
            ...
            user_table.append({"Invite Name": c.get('referee'), "1st Deposit": f"₱{c.get('deposit', 0):,.2f}", "Bonus": f"₱{c.get('amt', 0):,.2f}", "Status": display_status})
        st.table(user_table)
        for idx, c in enumerate(comms):
            if c.get('status') == "UNCLAIMED":
                if st.button(f"Request Bonus from {c['referee']}", key=f"req_{idx}"):
                    c['status'] = "REQUESTED"
                    data.setdefault('pending_actions', []).append({
                        "type": "COMMISSION_REQUEST", "amount": c['amt'], "referee": c['referee'], "comm_index": idx, "date": datetime.now().strftime("%Y-%m-%d %I:%M %p")
                    })
...
# --- LOGIN / REGISTER ---
    ...
    with t2:
        ...
        # MODIFIED: AUTO-FILL REFERRAL FROM LINK
        rn = st.text_input("REFERRAL NAME", value=current_ref).upper().strip()
        if st.button("REGISTER"):
            reg = load_registry()
            if not fn or len(p1) != 6 or p1 != p2: st.error("Check fields and PIN match (6 digits).")
            elif rn not in reg: st.error(f"Referral '{rn}' not found.")
            else:
                referrer_data = reg[rn]
                if not referrer_data.get('inv') or len(referrer_data.get('inv')) == 0: st.error(f"'{rn}' is not an active investor.")
                else:
